...
export const getProductsByPage = defineAction({
  ...
  handler: async ({ page, limit }) => {
    ...
    if (page > totalPages) {
      // También podríamos retornar el total de páginas -> page = totalPages;
      return {
    ...
    const productsQuery = sql`
      select a.*,
      (select GROUP_CONCAT(image,',') from (select * from ${ProductImage} where productId = a.id limit 2)) as images
      from ${Product} a
